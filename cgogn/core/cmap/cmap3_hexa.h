...
protected:
	...
	{
		const uint32 first = (d.index/24)*24;
...
public:
	...
	inline void foreach_incident_vertex2(Volume w, const FUNC& func) const
	{
		...
		if (!internal::void_to_true_binder(func, Vertex2(Dart(first++)))) return; // 0
		if (!internal::void_to_true_binder(func, Vertex2(Dart(first++)))) return; // 1
		if (!internal::void_to_true_binder(func, Vertex2(Dart(first++)))) return; // 2
		if (!internal::void_to_true_binder(func, Vertex2(Dart(first)))) return; // 3

		first += 17;
		if (!internal::void_to_true_binder(func, Vertex2(Dart(first++)))) return; // 20
		if (!internal::void_to_true_binder(func, Vertex2(Dart(first++)))) return; // 21
		if (internal::void_to_true_binder(func, Vertex2(Dart(first++)))) // 22
	...
	inline void foreach_incident_edge2(Volume w, const FUNC& func) const
	{
		static_assert(is_func_parameter_same<FUNC, Edge2>::value, "Wrong function cell parameter type");
		uint32 first = (w.dart.index/24)*24;
		if (!internal::void_to_true_binder(func, Edge2(Dart(first++)))) return;// 0
		if (!internal::void_to_true_binder(func, Edge2(Dart(first++)))) return;// 1
		if (!internal::void_to_true_binder(func, Edge2(Dart(first++)))) return;// 2
		if (!internal::void_to_true_binder(func, Edge2(Dart(first)))) return;  // 3
		first += 2;
		if (!internal::void_to_true_binder(func, Edge2(Dart(first)))) return;// 5
		first += 4;
		if (!internal::void_to_true_binder(func, Edge2(Dart(first)))) return;// 9
		first += 4;
		if (!internal::void_to_true_binder(func, Edge2(Dart(first)))) return;// 13
		first += 4;
		if (!internal::void_to_true_binder(func, Edge2(Dart(first)))) return;// 17
		first += 3;
		if (!internal::void_to_true_binder(func, Edge2(Dart(first++)))) return;// 20
		if (!internal::void_to_true_binder(func, Edge2(Dart(first++)))) return;// 21
		if (internal::void_to_true_binder(func, Edge2(Dart(first++))))// 22
	...
	inline void foreach_incident_face2(Volume w, const FUNC& func) const
	{
		static_assert(is_func_parameter_same<FUNC, Face2>::value, "Wrong function cell parameter type");
		uint32 first = (w.dart.index/24)*24;
		if (!internal::void_to_true_binder(func, Face2(Dart(first)))) return; // 0
		first += 4;
		if (!internal::void_to_true_binder(func, Face2(Dart(first)))) return;// 4
		first += 4;
		if (!internal::void_to_true_binder(func, Face2(Dart(first)))) return;// 8
		first += 4;
